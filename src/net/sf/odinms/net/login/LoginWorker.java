		...
		try {
			...
			for (int i = 0; i < possibleLogins; i++) {
				...
				if (client.finishLogin(true) == 0) {
					client.getSession().write(MaplePacketCreator.OnCheckPasswordResult(client)); // Auth request, authorize account
                                        //client.getSession().write(MaplePacketCreator.OnCheckUserLimitResult()); // Check user limit and continue login
                                        client.getSession().write(MaplePacketCreator.getServerList()); // Grab world serverlist (only Tespia)
                                        client.getSession().write(MaplePacketCreator.getEndOfServerList()); // End serverlist packet and finish login
