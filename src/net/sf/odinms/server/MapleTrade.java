...
public class MapleTrade {
	...
	public void complete2() {
		...
		if (exchangeMeso > 0)
                    chr.gainMeso(exchangeMeso, true, true, true);
		// just to be on the safe side...
		exchangeMeso = 0;
		if (exchangeItems != null)
                    exchangeItems.clear();
		chr.getClient().getSession().write(MaplePacketCreator.getTradeCompletion(number, true));
	}
	...
	public static void completeTrade(MapleCharacter c) {
		...
		if (partner.isLocked()) {
			...
			if (!local.fitsInInventory() || !partner.fitsInInventory()) {
				cancelTrade(c);
				c.getClient().getSession().write(MaplePacketCreator.getTradeCompletion((byte)0, false));
				partner.getChr().getClient().getSession().write(MaplePacketCreator.getTradeCompletion((byte)1, false));
				return;
			}
			local.complete2();
			partner.complete2();
