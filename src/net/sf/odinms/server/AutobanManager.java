	...
	public synchronized void addPoints(MapleClient c, int points, long expiration, String reason) {
		...
		if (this.points.get(acc) >= AUTOBAN_POINTS) {
			...
			if (c.getPlayer().isGM()) {
				...
				try {
					c.getChannelServer().getWorldInterface().broadcastMessage(null, MaplePacketCreator.serverNotice(0, "[Autoban] " + name + " banned by the system (Last reason: " + reason + ")").getBytes());
