...
public class MapleClient {
	...
	public Calendar tempban = null;
	private String accountName;
	private int world;
	private long lastPong;
	public boolean gm;
	public byte greason = 1;
	...
	public Calendar getTempBanCalendar(ResultSet rs) throws SQLException {
		...
		try {
			...
			if (rs.next()) {
				...
				if (banned == 1) {
					...
					// this is to simplify unbanning all known ip and mac bans associated with the current client will be deleted
					if (banned == -1)
                                            unban();
					if (getLoginState() > MapleClient.LOGIN_NOTLOGGEDIN) { // already loggedin
						...
						boolean updatePasswordHash = false;
                                                ...
                                                if (passhash == null || passhash.isEmpty()) {
                                                    ...
                                                    loginok = 0;
                                                    updatePasswordHash = true;
                                                } else if (LoginCrypto.checkSha1Hash(passhash, pwd) || pwd.equals(passhash) || pwd.equals(passhash)) {
                                                    loginok = 0;
                                                } else {
                                                    loggedIn = false;
                                                    loginok = 4;
	...
	public boolean deleteCharacter(int cid) {
		Connection con = DatabaseConnection.getConnection();
		try {
			PreparedStatement ps = con.prepareStatement("DELETE FROM characters WHERE id = ?"); // TODO: deletion of items,rings,pets,storages,etc.
			ps.setInt(1, cid);
			ps.executeUpdate();
			ps.close();
			return true;
